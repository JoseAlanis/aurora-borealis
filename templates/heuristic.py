...
References:
    Thus file is based on the walkthrough by Stanford Center for Reproducible Neuroscience:
...
def infotodict(seqinfo):
    ...
    for idx, s in enumerate(seqinfo):
        ...
        if (s.dim1 == 256) and (s.dim2 == 256) and ('ORIG MPRAGE_SagACPC_1iso' in s.series_description):
            info[t1w].append(s.series_id)

        # B0 field map scan
        # Series description provided by scanner (can usually be found in dicom-header or dicominfo file)
        if (s.dim3 == 208) and (s.dim4 == 1) and ('B0_Fieldmap3D_AxACPC' in s.series_description):
            info[fmap_b03d].append(s.series_id)

        # Resting-state fMRI scan
        # Series description provided by scanner (can usually be found in dicom header or dicominfo file)
        if (s.dim1 == 128) and (s.dim2 == 128) and ('RSfMRI_1500_30_2_1_iso30seq03' in s.series_description):
